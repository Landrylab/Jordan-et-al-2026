...
experiment = "solid-selection"

# Paths to the different config files
sample_sheet_path = "../data/solid_selection/sample-sheet.xlsx"
wt_seq = "../data/solid_selection/F4_wt_seq.fasta"        # WT sequence after trimming

# Path to the folder containing the sequencing reads
folder_reads = "../data/solid_selection/"

# Paths to the folders where to save the intermediate dataframes and quality control figures
if not os.path.exists("../results/intermediate_data_solid-selection/plots/"):
    os.makedirs("../results/intermediate_data_solid-selection/plots/")

if not os.path.exists("../results/intermediate_data_solid-selection/dataframes/fastqc/"):
    os.makedirs("../results/intermediate_data_solid-selection/dataframes/fastqc/")

if not os.path.exists("../results/intermediate_data_solid-selection/dataframes/merged_reads/"):
    os.makedirs("../results/intermediate_data_solid-selection/dataframes/merged_reads/")
    
if not os.path.exists("../results/intermediate_data_solid-selection/dataframes/aggregated_reads/"):
     os.makedirs("../results/intermediate_data_solid-selection/dataframes/aggregated_reads/")

if not os.path.exists("../results/intermediate_data_solid-selection/dataframes/read_counts/"):
     os.makedirs("../results/intermediate_data_solid-selection/dataframes/read_counts/")

folder_qc_figures = "../results/intermediate_data_solid-selection/plots/"
folder_fastqc = "../results/intermediate_data_solid-selection/dataframes/fastqc/"
folder_merged = "../results/intermediate_data_solid-selection/dataframes/merged_reads/"
folder_agg = "../results/intermediate_data_solid-selection/dataframes/aggregated_reads/"
folder_read_counts = "../results/intermediate_data_solid-selection/dataframes/read_counts/"
...
o = "45"          # Maximum length of the overlap between the R1 and R2 reads (length before trimming)
O = "95"          # Minimum length of the overlap between the R1 and R2 reads
...
for Sample in list(info.index):
    ...
    file_out= folder_aligned +str(info.loc[Sample]['name'])+ '_solid-selection.needle'
...
def count_aligned(Sample):
    ...
    # Define filepath based on sample name
    filepath = folder_aligned + str(info.loc[Sample]['name']) + '_solid-selection.needle'
    print(filepath)
...
#plt.axhline(y=expected_read_count, color = "black", linestyle = "--", linewidth = 1, label = "Expected read count")
ax.set_ylim(0, 400000)        
ax.set_title("Total reads processed", fontsize=18)
ax.set_xlabel('Sample', fontsize=14)
ax.set_ylabel('Read count (thousands reads)', fontsize=14) 
ax.tick_params(axis = "both", labelsize=12)
